...
func _on_Node_close_request():
	self.queue_free()
	print('removing node')
	EditorSingleton.add_history(get_type(self.name), self.name, self.get_offset(), get_text(self.name), [], get_node('../').get_connections(self.name))
...
func _on_Node_dragged(from, to):
	...
	EditorSingleton.add_history(get_type(self.name), self.name, to, get_text(self.name), [], get_node('../').get_connections(self.name))
...
func _on_Node_line_changed(text):
	...
	if length > 0 and text[length - 1] == ' ':
		EditorSingleton.add_history(get_type(self.name), self.name, self.get_offset(), text, [], get_node('../').get_connections(self.name))
